...
class PanelSixteenScene:SKScene {
    
    var panelsLogic: PanelsLogic = PanelsLogic.shared
    
    var wrapper:SKNode = SKNode()
    var panel:SKSpriteNode = SKSpriteNode(imageNamed: "vignetta16")
    var tablet:SKSpriteNode = SKSpriteNode(imageNamed: "vignetta16-tablet")
    ...
    func addTablet(){
        self.tablet.zPosition = 2
        self.tablet.alpha = 0
        self.tablet.position.x = CGFloat(-110)
        self.tablet.position.y = CGFloat(-100)
        self.wrapper.addChild(self.tablet)
    }
    
    func enterAnimation() {
        let slideAction = SKAction.move(to: .zero, duration: 1.2)
        
        let tabletAction = SKAction.run {
            self.tablet.run(self.animation())
        }
        
        self.wrapper.run(SKAction.sequence([slideAction, tabletAction]))
    ...
    func exitAnimation() {
        ...
        let removeAction = SKAction.run {
            ...
            self.panelsLogic.currentPanel = 17
    ...
    func animation() -> SKAction{
        ...
        let action = SKAction.sequence([waitAction, fadeinAction, waitAction, nextButtonAppearAction])
